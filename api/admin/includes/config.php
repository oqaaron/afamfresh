...
define('PESAPAL_URL_REFUND',       PESAPAL_API_BASE . '/Transactions/RefundRequest');
...
// =============================================================
// The actual DSN-building + `new PDO(...)` logic lives in
// includes/db_connect.php, which also defines DB_HOST/DB_USER/DB_PASS/
// DB_NAME/DB_SOCKET. It throws PDOException on failure rather than
// handling it itself, so the same file can be reused by non-HTTP
// scripts (see scripts/run-migrations.php) that need to report a
// connection failure their own way instead of this file's
// header()+json+exit, which exits 0 and would hide the failure from a
// shell script's `set -e`.
try {
