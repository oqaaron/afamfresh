...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($orderId && $action === 'assign_rider') {
        ...
        if ($riderId) {
            ...
            // Refuse to dispatch a rider today for an order scheduled for a
            // future date. dashboard.php's Awaiting Riders count already
            // excludes these; this closes the same gap for the actual
            // assignment action, which had no such check before.
            $scheduledDate = $orderCheck['scheduled_delivery_date'] ?? null;
            if ($scheduledDate !== null && $scheduledDate > date('Y-m-d')) {
                header('Location: orders.php?assign_failed=1&reason=scheduled');
                exit;
            }
